...
public class ContactManager {
    ...
    public static String formatPhoneNumber(String raw) {
        // 1. Strip non-digits
        String noDigits= raw.replaceAll("[^0-9]", "");
        
        // 2. Check length and format
            // Regex to group: (3 digits)(3 digits)(4 digits) -> $1-$2-$3
        if (noDigits.length() == 10) {
            return noDigits.replaceFirst("(\\d{3})(\\d{3})(\\d{4})", "$1-$2-$3");
        }
    ...
    public static void main(String[] args) {
        ...
        for (Contact c : contacts) {
            String formattedPhone = formatPhoneNumber(c.getPhone());
            c.setPhone(formattedPhone);
